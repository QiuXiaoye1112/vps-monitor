...
def test_install_agent_resets_existing_traffic_state(tmp_path, monkeypatch) -> None:
    config_path = tmp_path / "agent.toml"
    traffic_state = tmp_path / "traffic-state.json"
    traffic_state.write_text('{"version":2,"tx":123,"rx":456}', encoding="utf-8")
    systemd_dir = tmp_path / "systemd"
    commands: list[list[str]] = []
    answers = {
        "中心 VPS IP": "192.0.2.10",
        "节点 ID（每台机器必须不同）": "node-1",
        "面板显示名": "Node 1",
        "通信 token": "secret",
        "上报间隔（秒）": "1",
    ...
    monkeypatch.setattr(manager, "title", lambda _: None)
    monkeypatch.setattr(manager, "pause", lambda *args, **kwargs: None)
    monkeypatch.setattr(manager, "ask_ip", lambda prompt, default="": answers[prompt])
    monkeypatch.setattr(manager, "ask_port", lambda *args: 8080)
    monkeypatch.setattr(manager, "ask", lambda prompt, *args, **kwargs: answers[prompt])
    monkeypatch.setattr(manager, "ensure_apt_packages", lambda *args, **kwargs: None)
    monkeypatch.setattr(manager, "ensure_venv", lambda *args, **kwargs: None)
    monkeypatch.setattr(manager, "install_launcher", lambda *args, **kwargs: None)
    ...
    manager.configure_agent(local=False, install=True)

    assert not traffic_state.exists()
    assert ["systemctl", "stop", manager.AGENT_SERVICE] in commands
    assert ["systemctl", "enable", "--now", manager.AGENT_SERVICE] in commands
    assert ["systemctl", "restart", manager.AGENT_SERVICE] in commands
