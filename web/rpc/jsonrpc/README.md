...
本目录是 monitor 前端面 RPC（JSON-RPC 2.0）的服务端实现。所有对外接口最终都收敛为一条统一管道：
...
| 命名空间 | 方法（文件） |
| --- | --- |
| `admin` | client CRUD、ping task、session/settings/weight、clipboard、provider（oidc）、task 查询、system（logs/cloudflared/exec/test）、xtermjs |
...
Agent v2 上报的核心逻辑统一到 `web/api/client/ingest.go`。
