...
class InitManager {
  ...
  private unsubscribeRealtimeEvents: (() => void) | null = null
  private statusEventTimer: ReturnType<typeof setTimeout> | null = null
  private pendingStatusUUIDs = new Set<string>()
  ...
  private async runStartupRequests(): Promise<boolean> {
    ...
    if (!nodesAvailable && healthResult.status === 'rejected') {
  ...
  /** 启动 WebSocket 连接和实时事件流。 */
  private startRealtimeConnections(): void {
    // 根据主题配置决定初始连接模式
  ...
  private startRealtimeEvents(): void {
    ...
    this.unsubscribeRealtimeEvents = subscribeRealtimeEvents((event) => {
      if (event.kind === 'status')
        this.queueStatusRefresh(event)
      else if (event.kind === 'metadata')
        void this.refreshClient(event.uuid)
    })
  }

  private async refreshClient(uuid: string): Promise<void> {
    if (this.destroyed || !uuid)
      return
    try {
      const client = await this.rpc.getNode(uuid)
      if (!this.destroyed)
        this.nodesStore.updateNodeClients({ [uuid]: client })
    }
    catch (error) {
      console.error('[InitManager] Realtime client update failed:', error)
    }
  }

  private queueStatusRefresh(event: RealtimeEvent): void {
    if (!event.uuid)
      return

    this.pendingStatusUUIDs.add(event.uuid)
    if (this.statusEventTimer)
      return

    this.statusEventTimer = setTimeout(() => {
      this.statusEventTimer = null
      void this.flushStatusEvents()
    }, 0)
  }

  private async flushStatusEvents(): Promise<void> {
    if (this.destroyed || !this.pendingStatusUUIDs.size)
      return

    const uuids = [...this.pendingStatusUUIDs]
    this.pendingStatusUUIDs.clear()
    try {
      const statuses = await this.rpc.getNodesLatestStatus(uuids)
      if (!this.destroyed)
        this.nodesStore.updateNodeStatuses(statuses)
    }
    catch (error) {
      console.error('[InitManager] Realtime status update failed:', error)
    }
  }
  ...
  destroy(): void {
    this.destroyed = true
    if (this.statusEventTimer) {
      clearTimeout(this.statusEventTimer)
      this.statusEventTimer = null
    }
    this.pendingStatusUUIDs.clear()
    this.unsubscribeRealtimeEvents?.()
    this.unsubscribeRealtimeEvents = null
    if (this.reconnectTimer) {
      clearTimeout(this.reconnectTimer)
      this.reconnectTimer = null
    }
