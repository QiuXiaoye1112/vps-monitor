...
const useNodesStore = defineStore('nodes', () => {
  ...
  function applyClient(node: NodeData, client: Client): boolean {
    ...
    const nextPingTaskOrder = Array.isArray(client.ping_task_order) ? client.ping_task_order : []
    if (!areNumberArraysEqual(node.ping_task_order, nextPingTaskOrder))
      node.ping_task_order = [...nextPingTaskOrder]
