...
def recent_metric_logs(limit: int = 80) -> list[dict[str, Any]]:
    ...
    with closing(connect()) as conn:
        rows = conn.execute(
            ...
            SELECT
                ...
                metrics.net_upload_bps,
                metrics.net_download_bps
            FROM metrics
            LEFT JOIN nodes ON nodes.id = metrics.node_id
            ORDER BY metrics.received_at DESC, metrics.id DESC
            LIMIT ?
            """,
            (safe_limit,),
        ).fetchall()
    ...
    for row in rows:
        ...
        latency_ms = None
        if collected_at and received_at:
            latency_ms = max(0, int((received_at - collected_at).total_seconds() * 1000))
        logs.append(
            {
                "id": row["id"],
                ...
                "net_upload_bps": row["net_upload_bps"],
                "net_download_bps": row["net_download_bps"],
