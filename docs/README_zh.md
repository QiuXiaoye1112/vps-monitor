...
Monitor 是一款轻量级的自托管服务器监控工具，旨在提供简单、高效的服务器性能监控解决方案。它支持通过 Web 界面查看服务器状态，并通过轻量级 Agent 收集数据。

> [!WARNING]
> Monitor 是一款自托管的监控/控制程序，仅应部署在你拥有或已获得授权管理的系统上。请勿将 Monitor 武器化，或在未获授权的情况下部署、访问、持久化、执行命令及从事其他滥用行为。关于现实中的滥用风险，可参考 Huntress 的分析：[Monitor C2 agent abuse](https://www.huntress.com/blog/monitor-c2-agent-abuse)。
> 用户需要自行承担部署和使用 Monitor 的责任。开发者不对未经授权或滥用行为及其后果承担责任。
> 在 Windows 端开启远程控制后，客户端会在每次用户登录时通过 Windows 通知提醒用户 Monitor 是一款远程控制软件。

[文档](https://monitor-document.pages.dev/) | [文档(镜像站 By Geekertao)](https://www.monitor.wiki) | [Telegram 群组](https://t.me/monitor_monitor)
...
已上架1Panel应用商店，应用商店-实用工具-Monitor 即可安装
...
1. 访问 Monitor 的 [GitHub Release 页面](https://github.com/monitor-monitor/monitor/releases) 下载适用于你操作系统的最新二进制文件。
2. 运行 Monitor：
   ```bash
...
> [!NOTE]
> 确保二进制文件具有可执行权限（`chmod +x monitor`）。数据将保存在运行目录下的 `data` 文件夹中。
...
2. 构建后端：
   ...
   cd monitor
   ```
   将步骤1中生成的静态文件复制到 `monitor` 项目根目录下的 `/web/public/defaultTheme/dist` 文件夹，并将 `monitor-theme.json` 与 `preview.png`/`perview.png` 复制到 `/web/public/defaultTheme`。
   ```bash
   go build -o monitor
   ```
3. 运行：
...
[Monitor 主题开发指南 | Monitor](https://monitor-document.pages.dev/dev/theme.html)

[在 Crowdin 上翻译 Monitor](https://crowdin.com/project/monitor/invite?h=cd051bf172c9a9f7f1360e87ffb521692507706)

## 客户端 Agent 开发指南

[Monitor Agent 信息上报与事件处理文档](https://monitor-document.pages.dev/dev/agent.html)
