...
远程监控节点可直接运行 `sudo vm`，在“常用操作”中选择“重置 Agent 设置（不重新安装）”。中心 VPS 安装本机 Agent 后也有“重置本机 Agent 设置（不重新安装）”。这些流程会重新询问首次配置时的全部参数，重写配置并重置 Agent 实际流量统计，然后测试上报和重启 Agent，不会重新安装依赖或服务。

“当前已使用流量”统一在中心 VPS 设置：`sudo vm` → “监控主机” → 选择本机或远程节点 → “设置当前已使用流量”。默认 0 GB，网页显示 Agent 上报流量与该值之和；该值在节点下一个流量账期开始时自动归零。
