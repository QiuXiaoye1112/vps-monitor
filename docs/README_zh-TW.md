...
Monitor 是一款輕量級的自託管伺服器監控工具，旨在提供簡單、高效的伺服器性能監控解決方案。它支援透過 Web 介面查看伺服器狀態，並透過輕量級 Agent 收集數據。

> [!WARNING]
> Monitor 是一款自託管的監控/控制程式，僅應部署在你擁有或已獲授權管理的系統上。請勿將 Monitor 武器化，或在未獲授權的情況下部署、存取、持久化、執行命令及從事其他濫用行為。關於現實中的濫用風險，可參考 Huntress 的分析：[Monitor C2 agent abuse](https://www.huntress.com/blog/monitor-c2-agent-abuse)。
> 使用者需要自行承擔部署和使用 Monitor 的責任。開發者不對未經授權或濫用行為及其後果承擔責任。
> 在 Windows 端啟用遠端控制後，客戶端會在每次使用者登入時透過 Windows 通知提醒使用者 Monitor 是一款遠端控制軟體。

[文檔](https://monitor-document.pages.dev/) | [Telegram 群組](https://t.me/monitor_monitor)
...
已上架 1Panel 應用商店，應用商店-實用工具-Monitor 即可安裝
...
1. 存取 Monitor 的 [GitHub Release 頁面](https://github.com/monitor-monitor/monitor/releases) 下載適用於你作業系統的最新二進位檔案。
2. 執行 Monitor：
   ```bash
...
> [!NOTE]
> 確保二進位檔案具有可執行權限（`chmod +x monitor`）。資料將保存在執行目錄下的 `data` 資料夾中。
...
2. 建置後端：
   ...
   cd monitor
   ```
   將步驟1中產生的靜態檔案複製到 `monitor` 專案根目錄下的 `/web/public/defaultTheme/dist` 資料夾，並將 `monitor-theme.json` 與 `preview.png`/`perview.png` 複製到 `/web/public/defaultTheme`。
   ```bash
   go build -o monitor
   ```
3. 執行：
...
[Monitor 主題開發指南 | Monitor](https://monitor-document.pages.dev/dev/theme.html)

[在 Crowdin 上翻譯 Monitor](https://crowdin.com/project/monitor/invite?h=cd051bf172c9a9f7f1360e87ffb521692507706)

## 客戶端 Agent 開發指南

[Monitor Agent 資訊上報與事件處理文檔](https://monitor-document.pages.dev/dev/agent.html)
